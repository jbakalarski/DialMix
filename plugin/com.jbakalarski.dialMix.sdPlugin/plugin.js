...
function render(context, target, rawSettings) { const configured = settings(rawSettings); const displayName = configured.targetName || target.name; const percent = Math.round(Math.max(0, Math.min(1, target.volume)) * 100); const volume = `${percent}%`; const mutedOpacity = target.muted ? 0.5 : 1; send('setFeedbackLayout', context, { layout: 'layouts/dialmix.json' }); send('setFeedback', context, { icon: { value: iconSource(target, configured), enabled: configured.showIcon }, source: { value: displayName, color: configured.sourceColor, enabled: configured.showSource }, volume: { value: volume, color: configured.volumeColor, opacity: mutedOpacity, enabled: configured.showVolume }, bar: { value: percent, bar_fill_c: configured.barColor, bar_bg_c: configured.barBackgroundColor, bar_border_c: configured.barBorderColor, opacity: mutedOpacity, enabled: configured.showBar } }); send('setTitle', context, { title: configured.showSource ? displayName : volume, target: 'software' }); }
async function resolveTarget(configured) { try { return await api(`/api/audio/targets/${encodeURIComponent(configured.targetId)}`); } catch { const targets = await api('/api/audio/targets'); return targets.find(target => target.name === configured.targetMatchName && target.type === configured.targetType) || null; } }
async function update(context, rawSettings) { const configured = settings(rawSettings); try { await ensureService(); const target = await resolveTarget(configured); if (target) render(context, target, target.id === configured.targetId ? configured : settings({ ...configured, targetId: target.id, targetMatchName: target.name, targetType: target.type })); } catch (error) { log(`Unable to update DialMix target ${configured.targetId}.`, error); } }
async function change(context, rawSettings, direction, ticks = 1) {
  ...
  try {
    while (pending.ticks !== 0) {
      ...
      const resolved = await resolveTarget(configured);
      if (!resolved) continue;
      const target = await api(`/api/audio/targets/${encodeURIComponent(resolved.id)}/volume/${direction}`, { method: 'POST', body: JSON.stringify({ step }) });
...
function handleHostMessage(raw) {
  try {
    ...
    const event = JSON.parse(typeof message === 'string' ? message : Buffer.from(message).toString('utf8'));
    const payload = event.payload || {};
    if (event.event === 'willAppear') {
      const configured = settings(payload.settings);
      log(`Action appeared for ${event.context} with target ${configured.targetId}.`);
      contexts.set(event.context, { settings: configured, controller: payload.controller });
      update(event.context, configured);
    }
    if (event.event === 'didReceiveSettings') {
      const current = contexts.get(event.context) || {};
      current.settings = settings(payload.settings);
      contexts.set(event.context, current);
      log(`Settings received for ${event.context}: ${current.settings.targetId}.`);
      update(event.context, current.settings);
    }
    if (event.event === 'propertyInspectorDidAppear' || event.event === 'getSettings') sendInspectorData(event.context);
    if (event.event === 'sendToPlugin') {
      const current = contexts.get(event.context) || {};
      current.settings = settings(payload);
      contexts.set(event.context, current);
      if (host && host.readyState === WebSocket.OPEN) host.send(JSON.stringify({ event: 'setSettings', context: event.context, payload: current.settings }));
      update(event.context, current.settings);
    }
    if (event.event === 'dialRotate') {
      const current = contexts.get(event.context) || {};
      const ticks = Number(payload.ticks || payload.delta || 1);
      const configured = settings(current.settings);
      log(`Adjusting ${configured.targetId} by ${ticks} tick(s).`);
      change(event.context, configured, ticks >= 0 ? 'increase' : 'decrease', ticks).catch(error => log('Unable to change target volume.', error));
    }
    if (event.event === 'dialDown' || event.event === 'keyDown') {
      const current = contexts.get(event.context) || {};
      const configured = settings(current.settings);
      if (configured.pressAction === 'toggleMute') resolveTarget(configured).then(target => target ? api(`/api/audio/targets/${encodeURIComponent(target.id)}/mute/toggle`, { method: 'POST' }).then(value => value ? render(event.context, value, configured) : update(event.context, configured)) : update(event.context, configured)).catch(error => log('Unable to toggle target mute.', error));
    }
