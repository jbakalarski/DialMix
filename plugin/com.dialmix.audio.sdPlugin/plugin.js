...
async function update(context, rawSettings) { const configured = settings(rawSettings); try { await ensureService(); const target = await api(`/api/audio/targets/${encodeURIComponent(configured.targetId)}`); if (target) render(context, target, configured); } catch (error) { console.error('Unable to update DialMix target:', error.message); } }
...
async function sendInspectorData(context) { try { await ensureService(); send('sendToPropertyInspector', context, { targets: await api('/api/audio/targets'), settings: contexts.get(context)?.settings || DEFAULTS }); } catch (error) { console.error('Unable to load DialMix targets:', error.message); } }
function handleHostMessage(raw) { const message = raw?.data ?? raw; const event = JSON.parse(typeof message === 'string' ? message : Buffer.from(message).toString('utf8')); const payload = event.payload || {}; if (event.event === 'willAppear') { contexts.set(event.context, { settings: settings(payload.settings), controller: payload.controller }); update(event.context, payload.settings); } if (event.event === 'didReceiveSettings') { const current = contexts.get(event.context) || {}; current.settings = settings(payload.settings); contexts.set(event.context, current); update(event.context, current.settings); } if (event.event === 'propertyInspectorDidAppear') sendInspectorData(event.context); if (event.event === 'sendToPlugin') { const current = contexts.get(event.context) || {}; current.settings = settings(payload); contexts.set(event.context, current); update(event.context, current.settings); } if (event.event === 'dialRotate') { const current = contexts.get(event.context) || {}; const ticks = Number(payload.ticks || payload.delta || 1); change(event.context, current.settings || DEFAULTS, ticks >= 0 ? 'increase' : 'decrease').catch(error => console.error(error.message)); } if (event.event === 'dialDown' || event.event === 'keyDown') { const current = contexts.get(event.context) || {}; const configured = settings(current.settings); if (configured.pressAction === 'toggleMute') api(`/api/audio/targets/${encodeURIComponent(configured.targetId)}/mute/toggle`, { method: 'POST' }).then(() => update(event.context, configured)).catch(error => console.error(error.message)); } }
function connect() { host = new WebSocket(`ws://127.0.0.1:${port}`); const register = () => host.send(JSON.stringify({ event: 'registerPlugin', uuid: PLUGIN_UUID })); if (typeof host.addEventListener === 'function') { host.addEventListener('open', register); host.addEventListener('message', event => handleHostMessage(event.data)); host.addEventListener('error', error => console.error('OpenDeck connection error:', error.message || error)); } else { host.on('open', register); host.on('message', handleHostMessage); } }
