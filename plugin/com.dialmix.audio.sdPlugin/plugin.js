...
function iconSource(target, configured) { const source = configured.customIcon || (configured.autoApplicationIcon && target.type === 'Application' && target.icon) || `data:image/svg+xml;base64,${Buffer.from((icons[target.type] || icons.default).replaceAll('currentColor', configured.iconColor)).toString('base64')}`; return fitIcon(source); }
